...
| P2 | A successful endpoint advances the overall retrieval timestamp while another reading remains old. The widget previously considered only the primary metric. | Per-metric time semantics and local-day rollover handling. Warnings follow the visible changing metrics; completed nightly records do not become stale live sensors. Regression tests cover partial endpoint updates and retained readings. |
...
| P2 | A connected installation with a missing/corrupt snapshot initializes with demo health readings. | All installations initialize without invented readings. First launch and widget gallery offer connection/setup; disconnect returns to an empty state. No demo action remains in the app. The widget also rejects a previous version’s persisted demo snapshot before the host has launched. Host regressions cover onboarding and cleanup. |
...
Widget gradients are opaque, and the root window has an explicit background. CI lets native controls settle before capturing. Widget sections use bounded containers so medium layouts retain their outer margins.
...
Completed sleep, nightly HRV/respiration, daily resting heart rate/SpO₂ and individual weight/VO₂ max records stay meaningful with their date. Their age alone does not trigger a stale-sensor warning or a request to sync a phone. Progress values such as steps and Body Battery retain sync-age handling. App details and accessibility retain the period or measurement date; when the source supplies no actual measurement time, they label the retrieval date instead. Desktop widgets omit routine dates for recent records as described below. SpO₂ labels explicitly identify the daily average. Tests cover migration, hidden legacy readings, dates, unchanged records and delayed progress values.
